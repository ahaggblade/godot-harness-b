...
func _ready() -> void:
    ...
    if error != OK:
        push_warning("GodotAgentRuntime failed to connect to %s:%d" % [_host, _port])
        return
...
func _parse_user_args() -> void:
    _consume_agent_args(OS.get_cmdline_user_args())
    if _port <= 0 or _token.is_empty():
        _consume_agent_args(OS.get_cmdline_args())

func _consume_agent_args(args: PackedStringArray) -> void:
    for arg in args:
        if arg.begins_with("--agent-host="):
            _host = arg.trim_prefix("--agent-host=")
        elif arg.begins_with("--agent-port="):
            _port = int(arg.trim_prefix("--agent-port="))
        elif arg.begins_with("--agent-token="):
            _token = arg.trim_prefix("--agent-token=")
        elif arg.begins_with("--agent-session-id="):
            _session_id = arg.trim_prefix("--agent-session-id=")
