...
func _invoke_hook(params: Dictionary) -> Dictionary:
    ...
    var resolved_method := _resolve_hook_method(hooks, hook_method)
    if resolved_method.is_empty():
        return {"ok": false, "message": "AgentHooks missing method %s" % hook_method}

    var result = hooks.callv(resolved_method, hook_args)
    return {"ok": true, "result": result}

func _resolve_hook_method(hooks: Node, hook_method: String) -> String:
    if hooks.has_method(hook_method):
        return hook_method

    var parts := hook_method.split("_", false)
    var pascal := ""
    for part in parts:
        pascal += part.capitalize()

    if hooks.has_method(pascal):
        return pascal
